    ...
    async def handle_message(self, topic: str, payload: str) -> Optional[CommandResult]:
        ...
        try:
            ...
            if payload.lower() in ["1", "true"] or param_definitions:
                ...
                logger.debug(f"Ignoring payload '{payload}' for topic '{topic}' (requires '1' or 'true')")
                return self.create_command_result(success=True, message="Command ignored due to payload value")
                
        except Exception as e:
            logger.error(f"Error handling MQTT message for topic {topic}: {str(e)}")
