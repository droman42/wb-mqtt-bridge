...
# The mock devices carry no capability maps, so reconciler plans resolve to zero
# actions — transition *content* is covered by test_scenario_switch_reconciler.py
# with real capability maps; this file covers manager-level behavior (loading,
# validation, persistence, role actions, live state).
SAMPLE_SCENARIOS = {
    "movie_mode": {
...
class TestScenarioManager:
    ...
    @pytest.mark.asyncio
    async def test_load_scenarios_rejects_sourceless_scenario(
        self, mock_device_manager, mock_room_manager, mock_store, tmp_path
    ):
        """A scenario without a thin `source` selection is skipped at load time (Bug 2:
        never fatal) — with the imperative sequence format removed it could never be
        activated, so validation rejects it with a clear error."""
        scenario_dir = tmp_path / "scenarios"
        scenario_dir.mkdir(exist_ok=True)
        ...
        assert "sourceless" in manager.scenario_load_errors
        assert "sourceless" not in manager.scenario_map
        assert "source" in manager.scenario_load_errors["sourceless"]
    ...
    @pytest.mark.asyncio
    async def test_switch_scenario_success(self, scenario_manager):
        """Switching activates the target scenario via the reconciler.

        The mock devices carry no capability maps, so the plan resolves to zero
        actions (with warnings); the manager still sets current_scenario, persists,
        and reports success. Transition content (actual device commands, ordering)
        is covered by test_scenario_switch_reconciler.py with real capability maps.
        """
