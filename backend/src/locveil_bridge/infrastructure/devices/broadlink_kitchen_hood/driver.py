...
from locveil_bridge.domain.devices.models import KitchenHoodState
from locveil_bridge.infrastructure.config.models import BroadlinkKitchenHoodConfig, StandardCommandConfig
from locveil_bridge.domain.devices.config import BaseCommandConfig
...
class BroadlinkKitchenHood(BaseDevice[KitchenHoodState]):
    ...
    async def shutdown(self) -> bool:
        ...
        try:
            ...
            await self.emit_progress("shutdown complete", "action_success")
            return True
        except Exception as e:
    ...
    async def handle_set_light(
        ...
        if await self._send_rf_code(rf_code):
            ...
            return self.create_mqtt_command_result(
                success=True,
                mqtt_topic="kitchen_hood/light/state",
                mqtt_payload=state,
    ...
    async def handle_set_speed(
        ...
        try:
            ...
            logger.error(error_msg)
            await self.emit_progress("Invalid speed level value", "action_error")
            return self.create_command_result(success=False, error=error_msg)
        ...
        if await self._send_speed_rf_code(level):
            ...
            return self.create_mqtt_command_result(
                success=True,
                mqtt_topic="kitchen_hood/speed/state",
                mqtt_payload=level,
