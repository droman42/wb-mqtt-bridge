...
- [ ] **DRV-30** `[P0]` `[release]` — **eMotiva driver hardening: notification-driven readiness gate + keepAlive watchdog + re-subscribe on recovery** (REL-3 rack sitting #1, findings F1+F4 — `docs/review/rel3_rack_findings_2026-07-10.md`; owner mandate 2026-07-10: scenario startup must be safe *as if ARC/CEC worked* — must-have pre-release). The 2026-07-10 incident: `set_input source1` fired **3.3 s into a CEC/ARC handshake** the driver had already been notified about (`input_source='arc'` at power-on +0.4 s) → XMC-2 firmware wedge, wall-unplug required, whole-evening scenario dead. Design (notification-driven per owner requirement — no blind delays; all timings measured on hardware 2026-07-10): **(a) post-power-on readiness gate** — after `power_on` (or observed Off→On), `set_input` holds until **notification quiescence** (~2 s silence over source/audio_input/video_input; clean power-up burst measured < 1 s ×3 samples), hard cap ~15 s so a lost packet never deadlocks a scenario; if the settled source is `arc` and the target isn't, proceed only after quiescence (switching away from *settled* ARC is unverified on HW — probe rides DRV-32). **(b) keepAlive watchdog** — subscribe `Property.KEEPALIVE` (protocol V3; interval device-advertised **7500 ms** in the transponder, already parsed by the library); ~3 missed intervals → `reachable=False` + fail-fast speakable errors (the incident teardown burned 2×9 s of blind no-ack retries). **(c) re-subscribe on recovery** — Emotiva subscriptions live in **device** memory and a wall-unplug wipes them (F4: the bridge stayed deaf until restarted); heartbeat loss→return triggers reconnect + re-subscribe + state refresh (subscribe-in-standby verified working). Zone2 toggle flap (10:58:22 On→Off→On in 1.4 s) recorded as an observation — check in passing.
...
- [ ] **SCN-14** `[P0]` `[release]` — **Reconciler gates: canonicalize the comparison + promote gate timeouts to failed steps** (REL-3 rack sitting #1, findings F2+F3 — `docs/review/rel3_rack_findings_2026-07-10.md`; must-have pre-release with DRV-30). **(a) Wire↔canonical mismatch in `_wait`:** the gate polls the device state field for the *canonical* target but compares the *raw wire* value — `living_room_tv.input` polls `'hdmi2'` while the LG driver stores `'HDMI2'`/`'HDMI_2'`, so the TV-input feedback gate has NEVER confirmed (every activation logged the 3000 ms timeout and burned the wait). Translate the observed state through the capability's value table (the same wire↔canonical machinery the catalog/driver stack uses) before comparing. **(b) Gate timeout = evidence, not noise:** on a `feedback:true` capability, a gate timeout means the device *reported* it never reached the target — record it as a **failed step** in the plan result (the surface `set_input` no-acks already use, threaded to the switch response and WB card), instead of "(proceeding optimistically)". Evidence: `tv_on_speakers` reported **success twice** (11:12, 11:19) while the `processor.input → 'arc'` gate correctly detected ARC never engaged and the sound never left the TV. Feedback-less capabilities keep the optimistic path (nothing to know). Expected post-fix: `tv_on_speakers` honestly fails until DRV-32 restores CEC — by owner decision.
