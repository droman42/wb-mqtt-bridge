...
- [ ] **CORE-9** `[P0]` `[release]` — **MQTT reconnect counter never resets → the bridge permanently loses MQTT after 5 lifetime disconnects** (REL-5 #2, `docs/review/rel5_pretag_review.md`). `infrastructure/mqtt/client.py:189` — `retry_count` accumulates across the whole process lifetime; on the 5th transient drop (broker restart / Wi-Fi blip, each self-healing) it reaches `max_retries=5`, logs "giving up," exits `_run_mqtt_client`, and nothing supervises or restarts it → the house becomes uncontrollable via MQTT with WB state silently desyncing until a manual process restart. On a permanently-running controller this is a when-not-if. Fix: reset `retry_count = 0` on each successful connect (bound retries per *episode*, not per process life); consider unbounded/backoff reconnect for a live controller. Add a test.
