...
- [ ] **DRV-26** `[P1]` `[release]` — **HVAC value tables: wire side must be the firmware's NUMERIC codes — today HVAC mode/fan/vane/widevane control is silently DEAD.** Diagnosed 2026-07-09 against the `../mitsubishi2wb` firmware source (authoritative: `mitsubishi2wb.ino` `hpSettingsChanged()`/`mqttCallback()`): the VWB-14 tables took the *label strings* as `wire` (`"COOL"`, `"<<"`), but the firmware publishes **numeric indices** and its command callback **silently ignores** (`else return`) any non-numeric payload. Consequence, both directions: **(read)** inbound echoes (`mode: "2"`) fail enum-parse → raw numerics in `/state` (voice/HvacPanel show `2` not `cool`); **(write)** outbound `set_mode/set_fan/set_vane/set_widevane` publish labels the firmware drops — the commands DO NOTHING (only power `1`/`0` and setpoint work). The definitive tables (fix data): **mode** 0=auto 1=dry 2=cool 3=heat 4=fan_only; **fan** 0=auto 1=quiet 2..5=speed_1..4; **vane** 0=auto 1=swing 2..6=pos_1..5; **widevane** 0=swing 1=far_left 2=left 3=center 4=right 5=far_right 6=split. Fix in the `hvac` profile + the 3 device configs (keep the existing canonical identifiers + trilingual labels; only `wire` changes) + the VWB-14 drift-guard expectations + tests both directions (`_coerce_mirror('mode','2')→'cool'`, `_translate_outbound('cool')→'2'`). **Catalog value tables change ⇒ golden re-pin — coordinate with the voice side** (fold into the pending `16eee0f2f7832995` re-pin so voice pins once). HW check at the rack: canonical `set_mode(cool)` → wire `2` publish → echo → state `cool` (the REL-3 «Кондиционер» item, now with the root cause pinned).

- [ ] **DRV-27** `[P2]` `[deferred]` — **ESP32-managed HVAC as a dedicated driver — design** (`design-then-implement`; the VWB-11-anticipated "ESP32ManagedDevice migration", user-directed 2026-07-09 after the retained-wipe incident). Treat the 3 mitsubishi2wb HVACs as a bespoke MQTT driver (still MQTT-only) instead of generic passthrough: **(1)** declared typed state (`power/mode/fan/vane/widevane/setpoint/room_temperature`) → persistence + **restore-at-boot for free** (VWB-18 machinery; passthrough's dynamic fields deliberately don't restore per DRV-25) — covers the broker-restart wipe for the bridge/voice/UI view; **(2)** firmware wire↔canonical tables live once in driver code, with a source-of-truth pointer at `mitsubishi2wb.ino` (`hpSettingsChanged`/`mqttCallback`; the user explicitly does NOT want firmware changes); **(3)** optionally a **bridge-owned WB virtual card** with proper enum meta — human-readable labels in the WB UI and wipe-proof (the WB service already re-publishes retained state on reconnect), superseding the ESP32's raw numeric card for humans (the bridge must NEVER write the ESP32's own value topics — ownership). Context that motivates it: **mosquitto persistence stays OFF on the WB7 by explicit user decision** (2026-07-09, discussed with the WB community), and the firmware republishes only `meta` on reconnect — so wipe-robustness must live above the broker. Design decides: driver contract shape, catalog/golden impact (device_class changes ⇒ re-pin), HvacPanel implications, relationship to the DRV-25 enrichment (which stays for heating_loop etc.). Gated on DRV-26 landing first (tables are needed either way).
