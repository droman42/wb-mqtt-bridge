...
- [ ] **DRV-35** `[P2]` `[deferred]` — **`ESP32/` tree handover: DELETE + retire DRV-7
  (PROD-15 bridge delegation, item 1b).** *(UNBLOCKED 2026-07-12 — repo-to-repo filing by the
  satellite session: import confirmed, the 34 git-tracked files @ bridge `a80322f` copied verbatim to
  `locveil-satellite/imports/bridge-esp32/` (satellite **DES-6**, commit `0d950a9`; provenance +
  mining rules in its `imports/README.md`).)* Ready to execute: DELETE the top-level `ESP32/` tree in
  one change (the 2026-07-08 verdict, reconfirmed by HK-4) + retire DRV-7 with a journal pointer to
  the satellite repo as the new home. Until the delete lands the tree stays frozen — no edits to it.
