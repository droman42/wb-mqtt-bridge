...
- [ ] **DRV-25** `[P2]` `[deferred]` — **Implement WB-passthrough readable/authoritative switch `power`** (implementation of the DRV-24 design, `docs/design/wb_passthrough_readable_power.md`; cross-repo golden re-pin). Land D1–D3 as one change: (D1) the three profiles (`light_switch`×24, `dimmable_light`×13, `power_switch`×2) get a stateful+readable `power` capability (`state_field: "power"` + a `fields[power]` value table `'1'↔'on'`) — **and set `reconcile` EXPLICITLY (default `false`)**, because `reconcile` defaults to `true` and these profiles don't override it, so the `momentary → stateful` flip would otherwise start driving believed-vs-desired power diffs on all 39 devices (a behaviour change beyond the read-path fix); (D2) a load-time enrichment so a device's bare `state_topics[field]` inherits the profile field's `type`/`values` (device-explicit wins) — declares the table once per profile, and also discharges the DRV-23 sibling where bare enum fields like `heating_loop` `mode` project as raw `'0'`; (D3) `_on_value_message` sets the declared top-level field (`power`) from the coerced canonical value (DRV-23's `model_dump` setdefault stays for non-declared keys). Regenerate `contracts/` (`wb-catalog` + openapi) — **the golden content-hash changes → voice re-pins**, sequence both pins as one landing. Tests per the design §5. Picks up alongside the voice features that need it (QUAL-68 / ARCH-39); no release-1 gate.
