...
- **2026-05-22** — **Phase 1 hardware-verified + merged to `main`; Phase 2 (monorepo) executed end-to-end.**
  ...
  - **Post-monorepo doc-staleness (found in the 2026-05-22 wrap-up audit):** `project.md` ("Two repos … in lockstep"), `conventions.md` ("these two repos" / "both repos" / "sibling `wb-mqtt-bridge` checkout"), and `ui_backend_contract.md` (sibling-checkout build model, "two layouts", "developed in lockstep") still describe the pre-monorepo two-repo world — rewrite to the monorepo (UI reads `../backend`; one layout). Also pin a sqlite-capable Python (`backend/.python-version` = 3.11.12) — the local `/usr/local/python3.11.4` lacks `_sqlite3`.
