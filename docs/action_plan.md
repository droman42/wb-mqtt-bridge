...
- [ ] **DRV-27** `[P2]` — **ESP32-managed HVAC as a dedicated driver — design** (`design-then-implement`). **REOPENED 2026-07-10** — the user reviewed `docs/design/mitsubishi_hvac_driver.md` and rejected two decisions: **(1)** the single `climate` capability — the map must decompose into per-domain action groups like the AV precedents (LgTv/kitchen hood/eMotiva: power, fan, …); **(2)** deriving all topics from one `mqtt_device` field — the device configs must carry **explicit MQTT topics per action**, and the 3 HVAC configs move OUT of `wb-devices/` (bespoke devices live at `config/devices/` root). Settled decisions stand: name `MitsubishiHvac`, tables in the class map only, no bridge WB card, icons → UI-16, heartbeat reachability, restore-at-boot. Deliverable: the revised design doc; DRV-28 (implementation) amended on re-close.

- [ ] **DRV-28** `[P1]` `[release]` — **Implement the `MitsubishiHvac` driver** (implementation of the DRV-27 design, `docs/design/mitsubishi_hvac_driver.md`; pulled into release scope by the user 2026-07-10 — implemented immediately after the design session; the voice golden re-pin consolidates on this landing). One change: **(1)** `MitsubishiHvac`/`MitsubishiHvacConfig`/`MitsubishiHvacState` (typed declared state: base `power` + mode/fan/vane/widevane canonical + setpoint/room_temperature floats + reachable) — declared fields ride VWB-18 restore-at-boot (the broker-wipe fix); **(2)** all topics derived from one config field `mqtt_device` (the firmware layout is fixed — design §2); **(3)** wire↔canonical translation via the ATTACHED capability map (class-map-only tables, user decision D3 — verify attach-before-echo ordering and pin with a test); **(4)** `classes/MitsubishiHvac.json` (climate capability + DRV-26 tables + `reconcile: false` explicit), `profiles/hvac.json` DELETED (no other users); **(5)** the 3 device configs shrink to identity + `mqtt_device`; **(6)** heartbeat reachability — `room_temperature` arrives every 45 s unconditionally, a watchdog flips `reachable` after ~3 silent intervals (the firmware has no LWT); **(7)** command path: canonical → numeric wire → `…/{control}/on`, idempotence via the standard `idempotence_skip` chokepoint honoring `force`/`assume_state`. Tests per the device-test recipe (design §7). **Contract:** device_class is catalog surface → golden bumps (supersedes `a4a2b1aed5f86447`) → **voice re-pins ONCE after this lands** (covers DRV-25+26+28); check whether `MitsubishiHvacState` enters an explicit persisted-state union in openapi. Backend image rebuild + WB7 redeploy; HvacPanel unchanged (device_id routing, canonical vocab stable). HW check rides REL-3 alongside the DRV-26 live set_mode.
...
- [ ] **VWB-32** `[P1]` `[release]` — **Publish the retained catalog-version topic at startup — it is MISSING from the live broker right now.** Found 2026-07-09 during the HVAC retained-wipe investigation: the WB7 rebooted (14:37 MSK) and mosquitto runs **without persistence** (kept OFF by explicit user decision), so every retained message was wiped; `bridge/catalog/version` (`presentation/api/routers/system.py:17`, published retained only from the `/reload` path at `system.py:221`) never came back — the bridge restarted after the wipe and nothing republishes it at boot. Voice's catalog-staleness gate reads this retained topic; it is currently blind, exactly when the golden is churning (`16eee0f2f7832995` + the DRV-26 bump). Fix: publish the retained catalog version during bootstrap startup (after MQTT connect), same value the catalog serves; keep the `/reload` publish. Consider re-publishing on MQTT reconnect too (a broker restart mid-run wipes it again — and stays wiped until the next bridge event; the reconnect hook makes the bridge self-healing here). Test: startup publishes retained version; reload still does.
...
- [ ] **UI-16** `[P2]` `[deferred]` — **Enum-value icons via the shared `IconResolver` — the AV mechanism, extended to canonical values** (DRV-27 design D8, user decision "same mechanism as AV"). AV button icons resolve UI-side by name through the renderer's `IconResolver` (the manifest `ActionIcon` is a placeholder hint — `ui_backend_contract.md` "Icons"). Extend the resolver to canonical enum VALUES (`cool` → snowflake, `heat` → sun, `swing`/`quiet`/`speed_N`/vane-widevane positions…) and replace `HvacPanel.tsx`'s hardcoded `SECTION_GLYPHS` map with it. No contract change, no emoji in config — canonical identifiers are the stable keys; any future enum-carrying device gets icons for free.
