...
- [ ] **VWB-18** `[P1]` `[house]` — **Restart-durability triad: stale `active_scenario` resurrection, device-state restore stub, toggle-power inversion.** Surfaced by the voice repo's QUAL-56 durability review (2026-07-02, `wb-mqtt-voice/docs/review/faf_durable_execution_review.md` Part 2); **all three claims verified against live code 2026-07-02** (`cross-repo-source-of-truth` intake check) — line refs exact. **(1) Deactivated scenario resurrects on restart (P1):** `ScenarioManager.deactivate()` clears `current_scenario` but never deletes/overwrites the persisted `active_scenario` key (the `StateRepositoryPort.delete` method has **zero call sites** in src; `_persist_state` only writes while a scenario is active — `domain/scenarios/service.py:441-472, 392-400`). Net: deactivate → bridge restart → `_restore_state` re-activates the scenario and powers the AV gear back on. Fix: clear the persisted key atomically with the in-memory clear — **in `deactivate()` only**: `shutdown()` *intentionally* leaves the key so an active scenario survives a bridge restart without touching hardware (`service.py:474-487`); don't break that. **(2) Persist-without-restore rot:** `device:{id}` state is written on every change but `DeviceManager.initialize()` restore is a logging placeholder ("later phase", `domain/devices/service.py:435-455`) — persisted state never re-hydrates `device.state`; design doc §7.1 ("assumed state is persisted") overpromises. **Aggravation found at verification:** `initialize_devices()` persists each successfully-set-up device's *boot-default* state (`devices/service.py:180-187`) **before** the restore stub ever runs (bootstrap order), so the last-good snapshot is clobbered at every boot — restore must hook in *before* that initial persist, not at the stub's call site. **Decision: implement restore** (not re-scope the doc): `shutdown()`'s own contract already promises "the assumed state is preserved across the restart", and QUAL-56's rule is "persist + restore + a restart test ship together, or not at all". **(3) Toggle-power inversion on restart:** blind toggle-capability devices reset assumed state to defaults; scenario restore then emits a toggle that can turn an actually-ON device **off** (`domain/scenarios/reconciler.py:190-202`). Fixed by (2): restored assumed state makes the reconciler diff a no-op for devices unchanged while the bridge was down (residual staleness — device flipped *while* the bridge was off — is inherent to blind devices, same as pre-restart). Definition of done: restart round-trip tests for both the scenario key and device state (the QUAL-56 rule).
