...
- [ ] **REL-5** `[P0]` `[release]` — **Pre-tag code review (acceptance-gate item 4's review half — split out of REL-3 2026-07-09).** A **review task** per `review-then-remediate`: run a thorough review of the release codebase (chat-scoped or via the `/code-review` skill), produce a **frozen review doc under `docs/review/`**, and **file the worth-acting-on findings as fresh plan tasks** — a finding is not scope until it has an ID (`single-task-ledger`). **P0/P1 remediation lands before the tag.** **Not HW-gated and not gated on the deployed bridge** — every code-touching `[release]` task (DRV-5, SCN-11, OPS-8, VWB-13, the DRV/SCN chain) has landed, so the review runs against the release-complete code **now**, off the rack, in parallel with waiting for a REL-3 rack window. Feeds **REL-4** (its remediation may change what the docs describe). Exit-criteria item 5 (the code-quality gate).
