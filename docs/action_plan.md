...
- [ ] **DRV-25** `[P2]` `[deferred]` — **WB-passthrough state → top-level fields + readable `power`** (implementation of the DRV-24 design, `docs/design/wb_passthrough_readable_power.md` — **revised 2026-07-09 to the converged mirror-retirement shape**; cross-repo openapi+golden re-pin). One landing. **(D1)** the three switch profiles (`light_switch`×24, `dimmable_light`×13, `power_switch`×2) get a stateful+readable `power` (`state_field:"power"` + `fields[power]` value table `'1'↔'on'`) with **`reconcile:false` set EXPLICITLY** (it defaults true / is unset today, so the momentary→stateful flip would otherwise start reconciling all 39). **(D2)** load-time enrichment so a bare `state_topics[field]` inherits the profile field's `type`/`values` (device-explicit wins; also fixes the DRV-23 sibling where `heating_loop` `mode` surfaces as raw `'0'`). **(D3) retire `mirrored`:** `WbPassthroughState` → `extra="allow"`, `_on_value_message` sets each coerced value as a **top-level** attribute, the idempotence guard reads `getattr(state,field)`, **DRV-23's `model_dump` projection is removed**, + a collision guard for `state_topics` keys shadowing reserved base fields. **Fallout:** `ui/HvacPanel.tsx` migrates from `state.mirrored.*` (raw) to top-level canonical fields (`config-ui-stays-functional`); `restore_state` skips dynamic fields (re-seeded at setup). **Contract:** `/state`+persisted_state change shape → **openapi AND golden regenerate** (`WbPassthroughState` is in the persisted-state union; `extra="allow"` → `additionalProperties`), UI types regen, **voice re-pins** — land the pins together. Supersedes DRV-23's projection (external read behaviour preserved; its 2 tests rewritten to direct-field asserts). Picks up alongside the voice features that need it (QUAL-68 / ARCH-39); no release-1 gate.
