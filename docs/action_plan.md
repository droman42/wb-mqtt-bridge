...
- [ ] **REL-4** `[P1]` `[release]` — **Release docs pass — user-facing documentation true at the release version, in the voice project's spirit.** Exit-criteria item 6; the **sole remaining `[release]` gate before the tag**. **Scope reconciled 2026-07-10 (owner):** the §0 "master-doc handover / plan-driven→architecture-driven" half is **descoped to the board** — it's a project-governance convention, cross-repo (both repos adopt it), and voice hasn't done it either; it defers alongside VWB-33/34. `project.md` was **archived** (not user-facing — voice keeps no vision doc; `docs/archive/project.md` with a banner). What remains is a bounded accuracy pass mirroring the voice repo's user-facing model (README front-door + QUICKSTART tester guide + `docs/architecture/*` with `overview.md` as entry + `docs/guides/*`; second-person, benefit-first, honest status, **no internal tracking language** per `user-facing-docs-are-done`): **(1)** verify every user-facing doc tells the truth at the release version — root `README.md`, `docs/architecture/*` (6), `docs/guides/*` (4), the sub-READMEs (`backend`/`ui`/`contracts`/`eval`/`ESP32`) — **and carries NO internal tracking language** (task IDs, `§P3.7` positional refs, plan/journal pointers — `no-ledger-ids-in-user-docs`); **`docs/planned/*` are NOT user-facing** (owner ruling 2026-07-10 — internal roadmap/design notes; excluded from the pass, and user-facing docs must not route readers into them); **(2) DOC-11 folds in** — `docs/architecture/ui.md`'s scenario-dispatch narrative still describes pre-SCN-6 dispatch (controls posted at role devices; `source` device contributes an input dropdown) → rewrite for per-room Scenario Manager + canonical-first; **(3) add a `docs/QUICKSTART.md`** — the bridge lacks the voice repo's tester/first-run guide (deployment stays in `ops/INSTALL.md`; this is the first-run framing); **(4)** review the 6 ADRs (`docs/adr/*`) against reality — annotate/supersede any outdated, don't rewrite decision history; **(5)** regenerate stale diagrams (`docs/images/*.dot` staled by the scenario-redesign / canonical-first / HVAC-driver changes) in the existing visual style. **Gated by REL-3 + REL-5** (both DONE). Last task before the tag; VWB-16 (`[release]`, voice TEST-18 fixtures) is the only other open `[release]` and sits off the critical path.
