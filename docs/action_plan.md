...
- [ ] **DRV-28** `[P1]` `[release]` — **Implement the `MitsubishiHvac` driver** (implementation of the DRV-27 design **rev. 2**, `docs/design/mitsubishi_hvac_driver.md`; pulled into release scope by the user 2026-07-10; the voice golden re-pin consolidates on this landing). One change: **(1)** `MitsubishiHvac`/`MitsubishiHvacConfig`/`MitsubishiHvacState` (typed declared state: base `power` + mode/fan/vane/widevane canonical + setpoint/room_temperature floats + reachable) — declared fields ride VWB-18 restore-at-boot (the broker-wipe fix); **(2)** `classes/MitsubishiHvac.json` with **SIX per-domain capabilities** (power {on,off} · mode/fan/vane/widevane {set {value}} · temperature {set {value}} + readonly room_temperature; all stateful/feedback/`reconcile:false`; the `{value}` param convention mirrors VWB-19 select-form), `profiles/hvac.json` DELETED; **(3)** the 3 device configs **move to `config/devices/` root** (out of wb-devices/) keeping the explicit per-action `commands`+bare `state_topics` schema (+ `device_category: appliance`; state field `temperature` → `setpoint`, topic keeps the firmware name) — the driver VALIDATES completeness at load; **(4)** wire↔canonical via the ATTACHED class map (tables in the map only — DRV-25 enrichment; pin attach-before-echo with a test); **(5)** heartbeat reachability (45 s `room_temperature` publish; watchdog flips `reachable` after ~3 silent intervals — firmware has no LWT); **(6)** command path via the standard `idempotence_skip` chokepoint (`force`/`assume_state`); **(7)** **`HvacPanel.tsx` REWORK** — dispatches the new six-capability vocabulary (`mode.set {value}` etc.) instead of `climate.*` (`config-ui-stays-functional`). Tests per design §7. **Contract:** golden bumps (supersedes `a4a2b1aed5f86447`) → **voice re-pins ONCE after this lands** (covers DRV-25+26+28; the canonical HVAC vocabulary changes — flag it in the re-pin note); check whether `MitsubishiHvacState` enters an explicit persisted-state union in openapi. Backend image rebuild + WB7 redeploy. HW check rides REL-3.
