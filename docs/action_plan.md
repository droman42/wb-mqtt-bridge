...
- [ ] **VWB-20** `[P1]` `[house]` — **Contract patch v1.1 — the voice review's schema gaps, PRE-PIN.** Filed 2026-07-04 off the wb-mqtt-voice-side review of the freshly committed `contracts/` (five gaps G1–G5; verified against live code at intake per `cross-repo-source-of-truth` — four confirmed, G5's proposed remedy corrected). **Sequencing is the point: land BEFORE voice's TEST-17 pins its copy** — the cheapest moment for shape changes is while the pin doesn't exist; heads-up given to hold the pin until this closes. Scope:
  - **(G1) Typed `CatalogParam`** (name/type/required/default/min/max/description/`values`/`unit`) replacing the schema-free `params: List[Dict]` — used by **both** producers (the §6 projection's `_spec_dict` AND the hand-built `scenario.set` in `_project_scenario_managers`, whose dict shapes currently disagree). Gives Irene a contract-backed parser and the drift guard pydantic-level teeth.
  - **(G3) Scenario localized names** — `ScenarioDefinition.name` is a single English string, so the scenario enum's value labels are EN-only (the flagship SCN-4 feature is not invocable in Russian). Add localized names to the scenario schema, author ru for the 9 configs, project ru/en labels into the value table.
  - **(G4) `unit` on param descriptors** — `_spec_dict` drops the native spec's `units` (the WB service already reads it — data path exists); author the missing units where configs lack them (`set_setpoint` → `°C`). Solves QUAL-35's «поставь двадцать два градуса» requirement without a fragile param↔field name join (the `temp`-vs-`setpoint` mismatch stays, documented).
  - **(G5, corrected) `apps.launch` documented as intentionally OPEN** — the launchable set is runtime-dynamic (installed apps), NOT bridge-known; a static enum would lie. Add a machine-readable `options_from: "apps"` hint on the param pointing at `GET /devices/{id}/options/apps`, and document the pattern in `contracts/README.md` for future dynamic-set params.
  - **(G2, schema half only) optional `aliases: {locale: [...]}`** on `CatalogDevice`/`CatalogRoom` + the config models feeding them — empty everywhere until VWB-21 authors the vocabulary (additive; ships in this patch so the shape is pinned once).
  - **(G-minor) decide empty-capability projection** — suppress capability husks with neither actions nor fields (TVs' select-form `input`, see VWB-19) or keep them as capability markers; pick one, document it.
  - Definition of done: re-dump golden + STAMP, openapi regen (CatalogParam appears under components.schemas), drift guard green, `config-ui-stays-functional` (openapi change → UI types regen + check + build).

- [ ] **VWB-21** `[P1]` `[house]` — **Alias vocabulary authoring (interactive session — the G2 flesh).** The spoken house says «свет»/«лампа»/«люстра»/«подсветка» for the same fixture and «зал»/«гостиная» for the same room; with exactly one label per locale per entity, Irene's resolver fuzzy-matches against a single surface and recall suffers at real-home scale (QUAL-35's name-collision concern). After VWB-20 pins the `aliases` schema: an interactive paste-flow-style session (the `wb_device_authoring_log.md` pattern) walking rooms + devices with the user supplying the *actual household vocabulary* — what family members actually call each fixture/room, per locale. Deliverable: authored `aliases` across `rooms.json` + device configs where spoken variants exist (not every device needs any), re-dumped golden, drift guard green. **The vocabulary is user knowledge — this task cannot be done solo.**
