...
- [ ] **OPS-26** `[P1]` `HW-GATED` — **`meta/driver` wire-identity cutover `wb_mqtt_bridge` → `locveil-bridge`** (board delegation **PROD-21**, council **HK-8**; `../locveil-commons/process/python-layout.md` §3 — wire/deployed identifiers are deployment identity, each needs its own owner-gated task). **Owner-gated**, rides the same deploy cycle as CORE-10/CORE-11 but is a **separate, separately-revertible commit** — it is the ONLY task allowed to change the retained `meta/driver` string. Retained qos=1 republish-in-place: **no broker migration**, no topic-content contract change; the only visible effect is one string in the WB device UI. Verify the persisted-state / retained-topic assumptions against the live controller before flipping (per §3). Do NOT bundle into CORE-10.
...
- [ ] **CORE-10** `[P1]` — **Rename the import package `wb_mqtt_bridge` → `locveil_bridge` + console-script renames + the deliberate `catalog-v1.7` cut** (board delegation **PROD-21**, council **HK-8**; normative design: `../locveil-commons/process/python-layout.md` §2 — satisfies design-then-implement). ~1 day, ONE tree churn. Reconciled 2026-07-13: bridge is **already src-layout** (`backend/src/wb_mqtt_bridge/`, tests outside the package) so NO layout move is needed — this task is the rename only. Scope: (1) `wb_mqtt_bridge`→`locveil_bridge` across the package dir + the **122** referencing `.py` files under `backend/src` + `backend/tests` (imports + strings + the `[project.name]`/wheel packaging + the import-linter contract module refs in `backend/pyproject.toml` + `backend/config/device-state-mapping.json` module paths + the CI job ref + docs); (2) console scripts `wb-catalog`→`locveil-catalog`, `wb-openapi`→`locveil-openapi`, **retire `wb-api`** (all callers in-repo → hard cut, no alias); (3) the deliberate **catalog contract v1.7** minor cut — the rename moves generated catalog symbol names, so bump the catalog STAMP + regen the UI OpenAPI types (`cd ui && npm run gen:api-types` then `npm run check && npm run build` per `config-ui-stays-functional`), one voice re-pin after this covers catalog v1.6+v1.7; (4) delete the **4 inert test `sys.path` shims**. LAW checks before commit: `lint-imports` from `backend/` (contract module paths updated), pyright, pytest. **NEVER touches** wire/deployed identifiers (`meta/driver` is OPS-26). Full keeper checklist baseline: PROD-21 board entry.

- [ ] **CORE-11** `[P1]` — **Config tree `backend/config/` → repo-root `config/` + Dockerfiles → root `docker/` with root build context** (board delegation **PROD-21**, council **HK-8**; normative: `../locveil-commons/process/python-layout.md` §1 — product data lives at repo root; config tree is `config/` singular org-wide, matching the WB7 runtime trees). ~½ day, **separate commits** from CORE-10. Reconciled 2026-07-13: config at `backend/config/`, Dockerfiles at `backend/Dockerfile` + `ui/Dockerfile`. Scope A (config move): `backend/config/` → root `config/` — loader + CLI default paths, ~15 test paths, `ops/update.sh` rsync line, the CI path filter, the **`config-master-tree`** invariant text in `CLAUDE.md` (bridge dialect — repo-owned, edit in place), one DRV-36 design-doc path line; **NO contract cut**. Scope B (Dockerfiles): → `docker/Dockerfile.backend` + `docker/Dockerfile.ui` built with **repo-root context** (4 CI workflow lines, merge the two `.dockerignore`s; image *runtime assets* — nginx templates/entrypoints — stay with `ui/` per §1). LAW checks before commit. Full baseline: PROD-21 board entry.
