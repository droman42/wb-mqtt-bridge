...
- [ ] **DRV-24** `[P2]` `[deferred]` — **Make WB-passthrough switch `power` an authoritative, readable state** (`design-then-implement`; follow-on to DRV-23, from the voice side's re-test 2026-07-09). The `power` capability on the **39** momentary-power WB-passthrough switch/relay devices (`light_switch` + relay plugs) is `kind: momentary` (write-only): the relay's real state sits in `state.mirrored['power']` as raw `'1'`/`'0'`, but top-level `state.power` stays the vestigial default `'off'` and the catalog advertises power as non-readable — so it reports the opposite of reality on every switched relay. Voice reads top-level `state.<field>` (ARCH-8 §5c) and needs authoritative switch state for three queued features: a spoken "is the light on?", relative adjustments (voice **QUAL-68**), and the **ARCH-39** force-confirm (reads believed state before offering to force). **Direction chosen 2026-07-09 (make it authoritative).** Design: (1) change `light_switch` (+ any relay-switch profile) power from `momentary` to a **stateful readable** capability with a `state_field` + a **value table** (`'1'↔'on'`, `'0'↔'off'`) so `_coerce_mirror` yields canonical (the established [[value-label-translation]] `wire/canonical/labels` pattern); (2) extend the DRV-23 projection to **map** `mirror.power` onto top-level `state.power` — a mapping, not a copy, and deliberately overwriting the vestigial declared `power` for WB-passthrough (never meaningfully set there). **Contract impact: power becomes a readable catalog field → the golden catalog changes → the voice side re-pins** (cross-repo coordination). **Scope pinned = WB-passthrough `power` capability ONLY** (verified 2026-07-09): NOT `kitchen_hood` (Broadlink, one-way, already exposes optimistic `light`/`speed` top-level), NOT HVAC (its on/off is the `mode` readable field, already projected by DRV-23), NOT AV (already authoritative top-level power), NOT IR (one-way, no feedback). All 39 have relay feedback (`state_topics.power`), so all 39 can be authoritative. **Related sibling (separate task if pursued):** value-label completeness — bare-`state_topic` enum fields already projected by DRV-23 (e.g. `cabinet_floor` `mode`) surface as raw `'0'`, not the catalog's canonical vocab; add value tables where an enum vocab is advertised. Per `design-then-implement`, deliverable = the design decision; implementation follow-up(s) filed on completion.
