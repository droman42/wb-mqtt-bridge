...
- [ ] **OPS-12** `[P1]` `[release]` — **Voice-style startup log rollover + fix the dead rotation
  cleanup.** User-requested at the rack (2026-07-07), pre-REL-2: adopt the voice repo's behavior —
  **each startup renames the existing log aside and begins a fresh file** (voice:
  `_rotate_log_file` in `irene/utils/logging.py`), while **keeping daily rotation** for permanent
  controller operation. Bridge specifics: (1) startup rename targets the **same filename family**
  the daily rotation uses (`service.log.<stamp>.log`), so VWB-28's evidence collector
  (`_collect_logs`: live file + newest `service.log.*` sibling) keeps working **unchanged**;
  (2) **fix the pre-existing retention bug found during analysis** — `setup_logging` sets a custom
  `TimedRotatingFileHandler.suffix = "%Y%m%d.log"` without updating `extMatch`, so
  `getFilesToDelete()` never matches and **`backupCount=30` deletes nothing** — rotated dailies
  accumulate unbounded on the controller; (3) add a startup prune for the whole `service.log.*`
  family older than the retention window (covers startup-renamed files, which the handler's own
  cleanup will never match). Unit tests for rename/prune; suite + gates green.
