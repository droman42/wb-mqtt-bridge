...
- Generated by the **`locveil-openapi`** CLI (`locveil_bridge/cli/dump_openapi.py`), which
...
- Device-state models are injected into `components.schemas` by
  `locveil_bridge/app/bootstrap.py::_install_openapi_with_state_models` (an additive
...
- **Regenerate and commit `openapi.json` whenever the API surface or any device-state
  model changes** (`locveil-openapi -o openapi.json`). A regression test
...
{
  "LgTv": {
    "stateClassImport": "locveil_bridge.domain.devices.models:LgTvState",
...
| You changed… | Do this |
|---|---|
| A REST endpoint or request/response model | `locveil-openapi -o openapi.json`; commit; (UI) `npm run gen:api-types`, commit `api.gen.ts` |
| A device-state model's fields | `locveil-openapi -o openapi.json`; commit |
| Added/renamed a device-state model | add to `OPENAPI_EXTRA_MODELS` + mapping; `locveil-openapi`; commit |
